...
# opposed to finding already present, so uninstall-macos.command can remove
...
# Auto-generated by install-macos.command. Records which system-wide tools
# this installer itself put in place (vs. found already installed), so
# uninstall-macos.command removes only what it added and leaves any
# pre-existing software alone. Safe to delete if you'd rather
# uninstall-macos.command not touch any system-wide tools at all.
INSTALLED_HOMEBREW=$INSTALLED_HOMEBREW
INSTALLED_MINICONDA=$INSTALLED_MINICONDA
INSTALLED_ROSETTA=$INSTALLED_ROSETTA
INSTALLED_SUMO=$INSTALLED_SUMO
INSTALLED_XQUARTZ=$INSTALLED_XQUARTZ
